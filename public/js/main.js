...
(function ($) {
    ...
    //     // total =
    //     $input.val(newVal);
    //     var itemId = $button.closest('.pro-qty').data('id');

    //     $button.parent().find('input').val(newVal);

    //     $.ajax({
    //         url: '/update-quantity', // Define route in Laravel
    //         method: 'POST',
    //         data: {
    ...
    proQty.on('click', '.qtybtn', function () {
        ...
        var $input = $proQty.find('input');
        var oldValue = parseFloat($input.val());
        var newVal = $button.hasClass('inc') ? oldValue + 1 : Math.max(oldValue - 1, 0, 1);

        var price = parseFloat($proQty.closest('tr').find('.price').text());
        var totalPrice = (price * newVal).toFixed(2);
        $input.val(newVal);
        $proQty.closest('tr').find('.totalPrice').text(totalPrice);
        var itemId = $proQty.data('id');
